...
class MultiChoicesParser(CharacterLevelParser):
    def __init__(self, list_of_choices : list[list[str]] | None):
        ...
        if list_of_choices is not None:
            ...
            config = CharacterLevelParserConfig(self.parser.alphabet)
        else:
            self.parser = None
            config = None
    ...
    def can_end(self) -> bool:
        return end_symb in self.parser.next() or self.parser.finished
    
    def cache_key(self):
        return hash((id(self.parser.where_am_i), self.parser.finished, self.parser.success))
